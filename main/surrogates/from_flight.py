...
def main():
    ...
    vel= [data['vel_x'], data['vel_y'], data['vel_z']]
    omega = [data['omega_x'], data['omega_y'], data['omega_z']]
    quat = [data['q_x'], data['q_y'], data['q_z'], data['q_w']]
    ...
    states = ca.vertcat([pos, vel, omega, quat])
    control = ca.vertcat([ailerons, elevator, rudder, thrust, thrust, thrust, flaps])

    alphas = aircraft.alpha(states, control).full().flatten()
    betas = aircraft.beta(states, control).full().flatten()
    q_bars = aircraft.qbar(states, control).full().flatten()

    # extract forces and moments
    forces = [data['Fx'], data['Fy'], data['Fz']]
    moments = [data['Mx'], data['My'], data['Mz']]

    # generate coeff dataset:
    new_dataset = pd.DataFrame(columns = ['q', 'alpha', 'beta', 'aileron', 
                                     'elevator', 'windtunnel', 'CX', 'CY', 'CZ', 
                                     'Cl', 'Cm', 'Cn'])
    new_dataset['q'] = q_bars
    new_dataset['alpha'] = q_bars
    new_dataset['beta'] = q_bars
    new_dataset['aileron'] = q_bars
    new_dataset['elevator'] = q_bars
    new_dataset['windtunnel'] = 0
    new_dataset['CX'] = forces[0] / (q_bars * aircraft.S)
    new_dataset['CY'] = forces[1] / (q_bars * aircraft.S)
    new_dataset['CZ'] = forces[2] / (q_bars * aircraft.S)
    new_dataset['Cl'] = moments[0] / (q_bars * aircraft.S * aircraft.b)
    new_dataset['Cm'] = moments[1] / (q_bars * aircraft.S * aircraft.c)
    new_dataset['Cn'] = moments[2] / (q_bars * aircraft.S * aircraft.b)

    new_dataset.to_csv(Path(DATAPATH) / 'processed' / 'flight_data.csv')
