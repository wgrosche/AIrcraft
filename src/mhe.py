...
def detect_waypoints_in_range(state, waypoints, horizon_time):
    """
    Detect waypoints within a certain range of the current state.
    """
    current_velocity = state[7:10]
    max_dist = horizon_time * np.linalg.norm(current_velocity)


    detected_waypoints = []
    distance_to_next_waypoint = np.linalg.norm(state[:3] - waypoints[0][:3])
    if distance_to_next_waypoint < max_dist:
        detected_waypoints.append((waypoints[0], True))
        if np.linalg.norm(waypoints[0][:3] - waypoints[1][:3]) < (max_dist - distance_to_next_waypoint):
            detected_waypoints.append((waypoints[1], True))
    else:
        detected_waypoints.append((waypoints[0], False))

    return detected_waypoints
    
        

def horizon_step(initial_state, horizon, waypoints, dynamics):
    opti = ca.Opti()
